...
class TicketPanelView(discord.ui.View):
    """Persistent view - το κουμπί στο κυρίως panel μήνυμα."""

    def __init__(self):
        super().__init__(timeout=None)

    @discord.ui.button(
        label="Άνοιγμα Ticket",
        emoji="🎫",
    ...
    async def open_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.send_message(
            "Επίλεξε την κατηγορία του ticket σου:",
...
class Tickets(commands.Cog):
    ...
    async def ticket_panel(self, interaction: discord.Interaction):
        ...
        if os.path.exists(config.TICKET_PANEL_LOGO_PATH):
            ...
            embed.set_thumbnail(url=f"attachment://{config.TICKET_PANEL_LOGO_FILENAME}")
    ...
    async def create_ticket(self, interaction: discord.Interaction, category_key: str):
        ...
        # Έλεγχος role-restricted κατηγορίας (π.χ. Order Ticket)
